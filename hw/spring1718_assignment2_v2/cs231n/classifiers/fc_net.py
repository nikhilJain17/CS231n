...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        reg_term = self.reg / 2 * (np.sum(np.square(self.params['W1'])) +
            np.sum(np.square(self.params['W2'])))
        
        loss, softmax_grads = softmax_loss(scores, y)
        loss = loss + reg_term
        dout = affine_backward(softmax_grads, cache3)
        grads['W2'] = dout[1] + self.reg * self.params['W2']
        grads['b2'] = dout[2]
        dout = relu_backward(dout[0], cache2)
        dout = affine_backward(dout, cache1)
        grads['W1'] = dout[1] + self.reg * self.params['W1']
        grads['b1'] = dout[2] 
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
